...
<Dialog.Root bind:open={navbarStore.collaborate}>
  <Dialog.Portal>
    <Dialog.Overlay
      class="fixed inset-0 z-50 bg-black/20 dark:bg-black/70"
    />
    <Dialog.Content
      class="fixed top-1/2 left-1/2 z-50 flex w-96 max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-4 rounded-lg bg-neutral-900 p-6 text-neutral-100 outline outline-neutral-700"
    >
      <Dialog.Close class="button absolute top-2 right-2"
          onclick={() => {
            navbarStore.setCollaborate(false);
            copied = false;
          }}>     
