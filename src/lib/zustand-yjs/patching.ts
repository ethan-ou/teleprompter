    ...
    switch (type) {
      ...
      case ChangeType.UPDATE:
        if (value instanceof Function === false) {
          if (sharedType instanceof Y.Map) {
            if (typeof value === "string") sharedType.set(property as string, stringToYText(value));
            ...
            if (typeof value === "string") sharedType.insert(index, [stringToYText(value)]);
            else if (value instanceof Array) sharedType.insert(index, [arrayToYArray(value)]);
            else if (value instanceof Object) sharedType.insert(index, [objectToYMap(value)]);
      ...
      case ChangeType.DELETE:
        if (sharedType instanceof Y.Map) sharedType.delete(property as string);
          ...
          const index = property as number;
          sharedType.delete(sharedType.length <= index ? sharedType.length - 1 : index);
      ...
      case ChangeType.PENDING:
        if (sharedType instanceof Y.Map) {
          patchSharedType(sharedType.get(property as string), newState[property as string]);
        } else if (sharedType instanceof Y.Array) {
          patchSharedType(sharedType.get(property as number), newState[property as number]);
        }
        break;
...
export const patchState = (oldState: any, newState: any): any => {
  ...
  const applyChanges = (state: string | any[] | Record<string, any>, changes: Change[]): any => {
    if (typeof state === "string") return applyChangesToString(state as string, changes);
    else if (state instanceof Array) return applyChangesToArray(state as any[], changes);
  ...
  const applyChangesToArray = (array: any[], changes: Change[]): any =>
    changes
      .sort(([, indexA], [, indexB]) => Math.sign((indexA as number) - (indexB as number)))
      .reduce((revisedArray, [type, index, value]) => {
        switch (type) {
          ...
          case ChangeType.PENDING: {
            revisedArray[index as number] = applyChanges(array[index as number], value);
            return revisedArray;
          }
  ...
  const applyChangesToObject = (object: Record<string, any>, changes: Change[]): any =>
    changes.reduce(
