...
const isDiffable = (v: any): v is Diffable => isArray(v) || isString(v) || v instanceof Object;

const isArray = (d: Diffable): d is Array<any> => d instanceof Array;

const isString = (d: Diffable): d is string => typeof d === "string";

const isRecord = (d: Diffable): d is Record<string, any> => !isArray(d) && !isString(d);
...
const getStringChanges = (a: string, b: string): Change[] => {
  if (a === b) return [];
    ...
    return b.split("").map((character, index) => [ChangeType.INSERT, index, character]);
  } else if (b.length === 0) {
...
const getArrayChanges = (a: Array<any>, b: Array<any>): Change[] => {
  ...
  for (let index = 0; index < a.length; index++) {
    ...
    if (b[bIndex] === undefined) changeList.push([ChangeType.DELETE, index, undefined]);
      ...
      const nextDiff = typeof b[bIndex + 1] === "undefined" ? [] : getChanges(value, b[bIndex + 1]);
...
const getRecordChanges = (a: Record<string, any>, b: Record<string, any>): Change[] => {
  const changeList: Change[] = [];
